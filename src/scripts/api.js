var API = {
  ...
  complete(params) {
    if (!("notFoundRedirect" in params)) params.notFoundRedirect = true;

    if (!("unauthorizedReload" in params)) params.unauthorizedReload = true;
    ...
    return (xhr) => {
      ...
      switch (status) {
        ...
        case 500:
          error();
          if (params.error) params.error(response, xhr);
          break;
        default:
          error();
          if (params.error) params.error(response, xhr);
      ...
      if (params.complete) params.complete(response, xhr);
    };
  },
  request(params) {
    var settings = {};

    if (!params.object) throw new Error('"Object" parameter is required');

    if (!params.method) throw new Error('"Method" parameter is required');
    ...
    settings.beforeSend =
      ...
      function (request) {
        request.setRequestHeader(
          "Secookie",
          params.cookie || localStorage.getItem("market_cookie")
        );
      };

    if (params.data) settings.data = JSON.stringify(params.data);
  ...
  upload(params) {
    ...
    if (!params.data) throw new Error('"Data" parameter is required');

    if (!params.object)
      settings.url = `${this.url}${this.version()}/Image/?section=${
        params.section
      }`;
...
if (process.env.NODE_ENV === "development") window.API = API;
